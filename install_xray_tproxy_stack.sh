...
TMP_XRAY_CFG="/tmp/xray.config.test.json"
...
install_pkg_if_missing() {
  ...
  if ! opkg list-installed | grep -q "^${pkg} "; then
    if [ "$DRY_RUN" = "1" ]; then
      info "[DRY RUN] Would install package: $pkg"
    else
...
backup_existing_files() {
  ...
  if [ "$DRY_RUN" = "1" ]; then
    info "[DRY RUN] Would back up existing Xray files to: $bdir"
    return 0
  fi
...
save_state() {
  if [ "$DRY_RUN" = "1" ]; then
    info "[DRY RUN] Would save installer state to: $STATE_FILE"
    return 0
  fi
...
run_generated_validation() {
  ...
  if xray_check "$cfg"; then
    ...
    err "FAIL: xray run -test -config $cfg"
  fi
  echo
}
...
generate_temp_files() {
  rm -f "$TMP_XRAY_CFG" "$TMP_XRAY_INIT" "$TMP_XRAY_TPROXY_INIT" 2>/dev/null || true

  if [ "$TRANSPORT" = "xhttp" ]; then
    write_xray_config_xhttp_to "$TMP_XRAY_CFG" "$SERVER_HOST" "$SERVER_PORT" "$UUID" "$PUBLIC_KEY" "$SHORT_ID" "$SNI" "$XHTTP_PATH" "$XHTTP_MODE" "$WAN_IF"
  else
    write_xray_config_tcp_to "$TMP_XRAY_CFG" "$SERVER_HOST" "$SERVER_PORT" "$UUID" "$PUBLIC_KEY" "$SHORT_ID" "$SNI" "$TCP_FLOW" "$WAN_IF"
  fi

  write_xray_init_to "$TMP_XRAY_INIT"
  write_xray_tproxy_init_to "$TMP_XRAY_TPROXY_INIT" "$LAN_IF" "$SERVER_HOST"
}

apply_real_files() {
  mkdir -p "$XRAY_CFG_DIR"
  cp "$TMP_XRAY_CFG" "$XRAY_CFG_FILE"
  cp "$TMP_XRAY_INIT" "$XRAY_INIT"
  cp "$TMP_XRAY_TPROXY_INIT" "$XRAY_TPROXY_INIT"
  chmod +x "$XRAY_INIT" "$XRAY_TPROXY_INIT"
}
...
install_or_upgrade_stack() {
  ...
  xray run -test -config "$TMP_XRAY_CFG" || die "Generated Xray config is invalid."
...
dry_run_generate_only() {
  ...
  if ! command -v xray >/dev/null 2>&1; then
    warn "[DRY RUN] xray is not currently installed on this router."
  ...
  if command -v xray >/dev/null 2>&1; then
    ...
    warn "[DRY RUN] Skipping xray -test because xray binary is not installed."
    echo
  fi
...
main() {
  while :; do
    ...
    case "${choice:-}" in
      ...
      3) dry_run_generate_only ;;
      4) exit 0 ;;
      *) warn "Unknown option. Please enter 1-4." ;;
    esac
  done
}
